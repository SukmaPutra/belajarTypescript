...
const PostItem = ({ id, title, content, author, userId, createdAt }: PostProps) => {
  ...
  return (
    <article className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition">
      <div className="p-4"> {/* ⭐ Ubah dari p-5 ke p-4 */}
        {/* --- HEADER BAGIAN ATAS (Profil & Tombol Opsi) --- */}
        <div className="flex items-start justify-between mb-3">
          {/* Info Pengguna (Avatar, Nama, Waktu) */}
          <div className="flex items-center space-x-4"> {/* ⭐ Ubah dari space-x-3 ke space-x-4 */}
            {/* Wrapper Avatar */}
            <div className="w-12 h-12 rounded-full border border-slate-200 bg-slate-100 flex items-center justify-center overflow-hidden flex-shrink-0"> {/* ⭐ Ubah dari w-10 h-10 ke w-12 h-12 */}
              {avatarUrl ? (
                <img
                  src={avatarUrl}
          ...
          <button 
            ...
            className="text-slate-500 hover:bg-slate-100 rounded-full p-1 transition" 
        ...
        {id ? (
          <Link to={`/post/${id}`} className="block group">
            <h2 className="text-lg font-semibold text-slate-900 mb-2 group-hover:text-blue-600 transition">
              {title}
            </h2>
            <p className="text-base leading-relaxed text-slate-700 mb-4"> {/* ⭐ Ubah dari text-sm ke text-base */}
              {content}
            </p>
          </Link>
        ) : (
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-200"> {/* ⭐ Ubah dari pt-2 border-slate-200/50 */}
          <button 
            onClick={handleActionClick}
            className="flex items-center gap-1.5 text-slate-500 hover:text-blue-600 px-2 py-1 rounded-lg hover:bg-slate-50 transition"
          >
          ...
          <button 
            onClick={handleActionClick}
            className="flex items-center gap-1.5 text-slate-500 hover:text-green-600 px-2 py-1 rounded-lg hover:bg-green-50 transition"
          >
          ...
          <button 
            onClick={handleActionClick}
            className="flex items-center gap-1.5 text-slate-500 hover:text-pink-600 px-2 py-1 rounded-lg hover:bg-pink-50 transition"
          >
